...
public:
    ...
    std::unique_ptr<PoDoFo::PdfMemDocument> tempDocument; // For resolveDocument temporary loads
    QString currentFile;
    mutable QRecursiveMutex mutex;
    QList<PdfImageInfo> lastListedImages; // Cache for findImageByName pointers
};
...
PoDoFo::PdfMemDocument& PdfEditorEngine::resolveDocument(const QString& path)
{
    if (d->document && d->currentFile == path) {
        return *d->document;
    }
    d->tempDocument = std::make_unique<PoDoFo::PdfMemDocument>();
    d->tempDocument->Load(path.toUtf8().constData());
    return *d->tempDocument;
}

PdfImageInfo* PdfEditorEngine::findImageByName(int pageIndex, const QString& xobjectName)
{
    d->lastListedImages = listImages(pageIndex);
    for (auto& img : d->lastListedImages) {
...
bool PdfEditorEngine::loadDocumentForEditing(const QString &filePath)
{
    ...
    try {
        newDoc->Load(filePath.toUtf8().constData());
...
#ifdef QT_DEBUG
        qDebug() << "PoDoFo Engine structurally saved document to:" << outputPath;
#endif
        return true;
    } catch (const PoDoFo::PdfError& e) {
#ifdef QT_DEBUG
                             ...
                             PoDoFo::PdfMemDocument* document) 
{
    ...
    while (reader.TryReadNext(content)) {
        if (content.GetType() == PdfContentType::Operator) {
            ...
            // Check if coordinates overlap with redaction masks
            ...
            // Recurse into Form XObjects drawn with the 'Do' operator
            ...
            // Re-serialize standard operators & operands
...
bool PdfEditorEngine::applyRedactions(int pageIndex, const QList<QRectF> &rects)
{
    ...
    try {
        ...
        // 1. Content stream surgery — only on parseable plain-text streams
        auto* contentsObj = page.GetContents();
        ...
        // 2. Visual overlays — always applied regardless of stream surgery success
        PoDoFo::PdfPainter painter;
        painter.SetCanvas(page);
        painter.GraphicsState.SetNonStrokingColor(PoDoFo::PdfColor(0.0, 0.0, 0.0));
        for (const auto& r : pdfRects) {
        ...
        // 3. Remove annotations intersecting redacted areas
...
bool PdfEditorEngine::linearizeDocument(const QString &outputPath)
{
    ...
    try {
        ...
        QPDF pdf;
        pdf.processMemoryFile("input.pdf", buffer.data(), buffer.size());
...
bool PdfEditorEngine::exportPdfA(const QString &outputPath, int conformanceLevel)
{
    ...
    try {
        ...
        // Add sRGB Output Intent (required for PDF/A conformance)
        auto& catalog = d->document->GetCatalog();
...
bool PdfEditorEngine::sanitizeDocument(const QString &outputPath)
{
    ...
    try {
        ...
        // 3. Remove Embedded Files and JavaScript
        if (catalog.GetDictionary().HasKey(PdfName("Names"))) {
            auto* namesObj = catalog.GetDictionary().GetKey(PdfName("Names"));
            if (namesObj && namesObj->IsDictionary()) {
                auto& namesDict = namesObj->GetDictionary();
        ...
        // 4. Remove potentially dangerous auto-execution keys from Catalog
        ...
        // 5. Strip /AA, /A, /PieceInfo, /Thumb, and /Metadata keys from all pages
        ...
        // 6. Remove AcroForm XFA key if present
        ...
        try {
            QPDF pdf;
            pdf.processFile(tempPath.toUtf8().constData());
            QPDFWriter writer(pdf, outputPath.toUtf8().constData());
            writer.setStaticID(true);                   // Purge unique document identifiers
            writer.setStreamDataMode(qpdf_s_compress);   // Re-compress data streams to strip dead elements
            writer.write();
                        ...
                        double a, double b, double c, double d, double e, double f)
{
    ...
    size_t cmEndPos = searchFrom + cmPos + 3; // past " cm"
...
bool PdfEditorEngine::moveImage(int pageIndex, const QString &xobjectName, double dx, double dy)
{
    ...
    try {
        ...
        PdfImageInfo* target = findImageByName(pageIndex, xobjectName);
...
bool PdfEditorEngine::resizeImage(int pageIndex, const QString &xobjectName,
...
bool PdfEditorEngine::replaceImage(int pageIndex, const QString &xobjectName,
