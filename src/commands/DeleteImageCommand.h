...
private:
    ...
    bool performApply(QString* err) {
        if (!m_engine || !m_doc) {
            if (err) *err = QObject::tr("Deleting image %1 failed: no document.").arg(m_name);
            return false;
        }
        ...
        if (m_backup.isEmpty()) {
            if (err) *err = QObject::tr("Deleting image %1 was refused: the page backup is missing; nothing was changed.")
                                 .arg(m_name);
            return false;
        }
