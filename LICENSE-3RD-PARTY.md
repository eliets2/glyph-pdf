...
| **quickjs-ng** | 0.15.0 (MSYS2 ucrt64, `mingw-w64-ucrt-x86_64-quickjs-ng`) | MIT | **Yes** | ECMAScript engine for run-side AcroForm Calculate/Format execution (form-JS Phase 1). In-process but sandboxed: 16 MiB memory cap, interrupt-handler execution deadline, zero host I/O (the quickjs-libc I/O module is never initialized), egress verbs (`submitForm`/`mailDoc`/…) blocked and audited. The AF host-object shim ports the Calculate/Format subset of Mozilla pdf.js `src/scripting_api/aform.js`, `util.js` and `shared/scripting_utils.js` (Apache-2.0, © 2020 Mozilla Foundation) with attribution; behavior pinned by golden tests in `tests/TestFormJsCalc.cpp`. Engine source: <https://github.com/quickjs-ng/quickjs> (MIT). |
