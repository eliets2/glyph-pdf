...
| PGR-40 | HIGH | quickjs-ng **0.15.0** (pinned dependency) | **CPU-deadline bypass:** native sparse-array scans never poll the interrupt handler and never allocate per hole — `Array(2^31).indexOf` 44.8 s, `includes` 53.2 s, `lastIndexOf` 56.6 s, `flat` 61.8 s, `sort(2^28)` 12.8 s, `join` 37 s, measured with the exact sandbox contract. One expression in any `/AA` script = unkillable UI freeze, repeatable per save/keystroke. | **Deferred — dependency bump.** Upstream added array-method interrupt checks after 0.15.0 (verified in master). **Checked 2026-09-24: MSYS2 0.15.1-1 does NOT carry the fix** — same-machine version-stamped A/B probe unkillable on both versions (pin probe 188 ms / 303 ms ok past a 150 ms deadline); 0.15.1 taken as a clean patch bump, zero golden drift. Probe in `docs/audit/evidence-formjs-2026-09-23/` (`qjs-probe-2026-09-24.c` + version-stamped captures); the suite's `nativeSparseArrayScansAbideTheDeadline` pin auto-arms when a fixed package lands |
