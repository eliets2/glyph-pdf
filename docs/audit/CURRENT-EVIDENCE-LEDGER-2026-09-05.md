...
| E-1-residual | PDF/A conformance validation was unverified; exported artifacts lacked DestOutputProfile ICC + complete CIDSet; veraPDF CLI wired as optional integration (structural contract always-on, CLI-validated pass with mismatch negative control) | PARTIAL — structural contract enforced for all 5 levels + real CLI validation wired; artifacts still fail 6.2.4.3-4 (missing ICC) and 6.3.5-3 (CIDSet); VeraPdfValidator::parseJson found schema-stale (separate defect, in repair) | fee597b |
