...
|----|----------|------------------------|--------|------|-----------------|----------|--------|---------------------|
| F01/R01 | forms | Failed form write leaves source PDF byte-identical; same-file add survives reopen | open | FormManager.cpp (all doc.Save sites), AddFormFieldCommand | pending | runtime repro (15,257→0 bytes) | — | — |
| F09/R02 | forms | Undo restores original value/tooltip/required incl. empty states | open | EditFormFieldCommand, FormManager | pending | trace | — | rename/placeholder/regex persistence contract out of scope |
| F02/R03 | AI | Timeout/destroy cannot produce late callback access; exactly one result | open | OllamaProvider.cpp:157-174 | pending | trace | — | no sanitizer run claimed |
